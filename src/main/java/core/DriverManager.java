...
public class DriverManager extends Helper {
    ...
    private void initializeDriver(String browserType) {
        logger.info("[DriverManager]: Initializing {} browser", browserType);
        ...
        if (webDriver.get() != null) {
            logger.warn("[DriverManager]: WebDriver already initialized for this thread");
            return;
        }
        ...
        try {
            ...
            logger.info("[DriverManager]: WebDriver initialized successfully for browser: {}", browserType);
        } catch (Exception e) {
            logger.error("[DriverManager]: Failed to initialize {} browser", browserType, e);
